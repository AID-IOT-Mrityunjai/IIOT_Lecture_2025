...
void setup() {
    ...
    log_v("Basic Multi Threading Arduino Example\n");
    // Now the task scheduler, which takes over control of scheduling individual tasks, is automatically started.
    log_d("Loop Core ID = %d",xPortGetCoreID());
...
void TaskBlink(void *pvParameters) {  // This is a task.
  ...
  */
    ...
    log_v("TaskBlink created");
    log_w("TaskBlink Core ID = %d",xPortGetCoreID());
...
void TaskAnalogRead(void *pvParameters) {  // This is a task.
    (void)pvParameters;
    log_d("TaskAnalogRead created");
    // Check if the given analog pin is usable - if not - delete this task
    if (digitalPinToAnalogChannel(ANALOG_INPUT_PIN) == -1) {
        log_w("TaskAnalogRead cannot work because the given pin %d cannot be used for ADC - the task will delete itself.\n", ANALOG_INPUT_PIN);
        analog_read_task_handle = NULL;  // Prevent calling vTaskDelete on non-existing task
  ...
  */
    log_d("TaskAnalogRead Core ID %d",xPortGetCoreID());
    for (;;) {
